...
<div class="container mt-4">
  ...
  <div>
    ...
    <button class="bt" @click="handleClick">出荷する</button>
    <div class="sand-effect" ref="sandEffect"></div>

    <!-- 「あ」をv-ifで表示を切り替え -->
    <div v-if="showA" class="move-text">
      <!-- トラックアイコン -->
      <i class="fa-solid fa-truck-fast" style="font-size: 10rem; color: #F4A300;"></i>
    ...
    methods: {
      ...
      handleClick() {
        if (this.clickCount % 2 === 0) {
          ...
          this.displayText = "";
          this.showA = false; // クリックのたびに「あ」を非表示にする
          ...
          this.$nextTick(() => {
            newPhrase.split("").forEach((char, i) => {
              ...
              span.style.animation = `disappear 0.25s forwards ${i * 0.1 + 0.1}s`; // 0.1秒後に消える

              // ★ これがポイント！最後の文字だけanimationendを監視する
              if (i === newPhrase.length - 1) {
                span.addEventListener('animationend', () => {
                  this.showA = true; // 最後の文字のアニメーションが終了したら「あ」を表示
                });
              }
